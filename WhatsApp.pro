...
UBUNTU_TRANSLATION_DOMAIN="whatsapp.nfsprodriver"
...
aptest.commands = QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/WhatsApp/tests/autopilot/run
aptest.depends  = sub-WhatsApp sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/WhatsApp/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-WhatsApp sub-backend-App
